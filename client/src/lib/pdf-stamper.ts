...
export async function stampPdfBuffer(
  ...
  stamp: PaymentStamp,
  t: Translations
): Promise<{ pdfBase64: string, pdfDataUri: string }> {
  try {
    ...
    return { pdfBase64, pdfDataUri };

  } catch (error) {
    console.error('Error stamping PDF buffer:', error);
    throw error;
  }
}
...
export async function addPaymentStampToPDF(
  ...
  try {
    ...
    // Use the core stamping logic
    const { pdfBase64, pdfDataUri } = await stampPdfBuffer(arrayBuffer, stamp, t);
    ...
    if (window.electronAPI && businessConfig.stampedInvoiceSavePath) {
      try {
        const fullPath = `${businessConfig.stampedInvoiceSavePath}\\${stampedFileName}`;
        ...
        if (result.success) {
          ...
          // Return the data URI anyway so they can see the preview or try again?
          // Actually, if they canceled save, they probably don't want to proceed with "Success" toast.
          // Throwing an error or returning null might be better, but the current flow expects a string.
          // However, the caller effectively "finished" the stamp process.
          // Let's just return the data URI, so the preview still updates.
          return pdfDataUri;
        }
      } catch (err) {
    ...
    if (isNative) {
      try {
        ...
        // Share the file using its native URI
        await Share.share({
...
export async function previewPDFStamp(
  ...
  try {
    ...
    const pdfBytes = await pdfDoc.save();
    const pdfBase64 = arrayBufferToBase64(pdfBytes.buffer as ArrayBuffer);
    return `data:application/pdf;base64,${pdfBase64}#toolbar=0&navpanes=0&scrollbar=0`;
  } catch (error) {
